...
function MainContentFilteredList() {
  ...
  let isPriceCategory = (checkedItem) =>
    checkedItem.id !== -1 && checkedItem.parentOrder === 0;
  let isScreenSizeCategory = (checkedItem) =>
    checkedItem.id !== -1 && checkedItem.parentOrder === 1;
  ...
  const handleDeleteFilter = (e) => {
    ...
    if (listDataset.type === 'all') {
      ...
      let isNotDeleteCheckedItem = (e) =>
        !(
          e.id === +listDataset.id && e.parentOrder === +listDataset.parentOrder
        );
      let newCheckedItems = checkedItems.filter(isNotDeleteCheckedItem);
      let parentOfDeletedHasChildren =
        newCheckedItems.filter(
          (e) => e.parentOrder === +listDataset.parentOrder
        ).length > 0;
  ...
  return (
    <div className="main-content-filter-list" id="products">
      <div className="main-content-filter-list-header flex ali-center">
        <h3>
          Laptop {brandName} {price} {screenSize}
        </h3>
